...
def get_tweepy_authentication_token():
...
def main():
    ...
    results_df = pd.DataFrame(columns=['ID', 'Text', 'SentimentLabel'])

    TWEEPY_API_INSTANCE = get_tweepy_authentication_token()

    for id_processing_batch, sentiment_processing_batch in get_batch_to_process():

        try:
            # Recover the full text
            results = TWEEPY_API_INSTANCE.statuses_lookup(id_processing_batch)
            ...
            for index, tweet in enumerate(results):
                ...
                tweet_sentiment_label = str(sentiment_processing_batch[index])
                ...
                results_df = results_df.append({'ID': tweet_id, 'Text': tweet_text,
                                                'SentimentLabel': tweet_sentiment_label}, ignore_index=True)
    ...
    results_df.to_excel(
        r'C:\Dev\bootcamp\twitter_api_exercise\docs\excel_pandas_out.xlsx', index=False)
